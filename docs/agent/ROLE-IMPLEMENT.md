...
### 裁量登记
**裁量登记**（DGA-A A-22；ADR-0107）
规格或意图沉默、必须自行做出选择时：先估三因子（风险 × 不可逆性 × 新颖度）。
高风险且不可逆且首次 → 停，升级 owner，无裁量。
其余情形可执行，但选择必须记入产出：沉默位置 + 所作选择 + 三因子结论。
隐瞒裁量即缺陷；申报的分歧是组织的规格缺口信号，不是个人失分项。
选择事后被推翻的，在原登记处追加一行复盘注记（改判理由入判例素材）。
...
### 负结果查询
**负结果查询**（DGA-A A-40；ADR-0107）
尝试任何新方法、新路径、新工具前：查 archive/neg-results/（INDEX + 条目 grep）。
命中且过期条件未满足 → 不得重试，除非携带新正当性依据（新能力/新约束）并声明。
命中且过期条件已满足 → 本次尝试即重验证实验：按实验标准记录，结果回写（复活或维持）。
未命中 → 正常执行。
自 neg-results 库建成（IR-0009 C1）起生效；建成前以 ADR 索引 + REMOVAL 检索替代。
