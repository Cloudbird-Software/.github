...
6. 红队守门（ADR-0082）：意图→spec→测试设计路径的 PR 必须经红队审计并作为合并阻断项（BEH-01）；verdict=insufficient 时机器阻断，survived 才放行。默认 verifier 范式（ADR-0072）+ endpoint 三探测（AC-10）+ metering 交叉核对（AC-11）+ fail-closed 总原则（INV-04）。CNB 执行通道（pipeline/adversary/cnb_bridge.py）与 fallback 链（CNB→自有 API→no-attempts）；凭据纪律：CNB 沙箱零 GitHub 凭据（INV-02/AC-6）
7. g060 制度（ADR-0061/0081）：`specs/*/suite/**` 的 CODEOWNER = verifier-app[bot] + owner（W2-C2）；非授权身份修改由 `scripts/g060-lock.sh` exit 2 阻断并自动开裁决 issue 路由 owner（终态机器可核 + TTL + dead-man 提醒，`scripts/g060-escalation.py`）；开发 agent 修改测试仍触发 fail-escalate
8. front-desk 命令（卡 issue 评论，conductor 转介 arbiter 处理）：/claim 认领 · /release 释放租约 · /retry 隔离回流 · /g060-adopt · /g060-reject（g060 裁决）
